% El método falsa posicion se utiliza para encontrar la raíz de una ecuación no lineal en 
% metodos numericos. Este es un método cerrado porque en cada iteración tenemos que verificar 
% el signo de la función. Dado que la raíz se encuentra  dentro del intervalo en el dominio, 
% es por eso que también se conoce como método de horquillado. Este método es más rápido que 
% el método de bisección y más lento que el método de Newton Raphson. Da la garantía de converger 
% una raíz única. El método de posición falsa requiere dos suposiciones iniciales para iniciar 
% la solución y no es necesario encontrar la derivada de la función.

%La fórmula del método Regula falsi es x = (af(b)-bf(a))/(b-a)

% Aquí, escribimos el código del Método de posición falsa en MATLAB paso a paso. MATLAB es una 
% manera fácil de resolver problemas complicados que no se resuelven a mano o que son imposibles 
% de resolver en la página.
